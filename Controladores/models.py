...
class Acudiente(db.Model):
    ...
    ID_Usuario = db.Column(Integer, ForeignKey('Usuario.ID_Usuario'), nullable=False)
...
class Notificacion(db.Model):
    ...
    ID_Notificacion = db.Column(Integer, primary_key=True, autoincrement=True)
    Titulo = db.Column(String(150), nullable=False)
    Mensaje = db.Column(String(300), nullable=False)
    Enlace = db.Column(String(255))
    Estado = db.Column(Enum('Leída', 'No leída'), default='No leída')
...
class Curso(db.Model):
    ...
    docente_asignaturas = relationship('Docente_Asignatura', back_populates='curso', lazy='dynamic')
...
class Docente_Asignatura(db.Model):
    ...
    ID_Curso = db.Column(Integer, ForeignKey('Curso.ID_Curso'), nullable=True)
    
    curso = relationship('Curso', back_populates='docente_asignaturas')
